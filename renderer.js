...
var udpPort = new osc.UDPPort({
    localAddress: config.get('udp').localAddress || '0.0.0.0',
    remoteAddress: config.get('udp').remoteAddress || '0.0.0.0',
    localPort: config.get('udp').localPort || 7000,
    remotePort: config.get('udp').remotePort || 7001,
    broadcast: config.get('udp').broadcast || true
})
udpPort.open()
...
function scan() {
    ...
    SerialPort.list().then(ports => {
        ports.forEach(port => {
            ...
            option.selected = port.path.includes('usbmodem')
...
function onData(str) {
    ...
    if (data.length === 2) {
        ...
        udpPort.send(dataObject, udpPort.ip || '127.0.0.1', udpPort.remoteAddress)
    }
}
